#!/usr/bin/env python3
"""
Generate scenario configurations from siting data - Combined Multi-Location version.

In this model, ALL locations from a single experiment are combined into ONE scenario.
Each location gets a data center with its colocated generation.

Example: If experiment has 10 locations, the scenario will have:
- 10 data centers (each at different bus)
- 10 sets of colocated solar/wind/storage

Usage:
    python generate_siting_scenario_combined.py <siting_csv> [--periods winter,summer]
    
Examples:
    python generate_siting_scenario_combined.py ../data/siting_data/100MW_onlyren_nameplate_capacities.csv
"""
...
def create_combined_scenario(
    ...
    """
    Create scenario with ALL locations from one experiment combined.
    """
    global GEN_COUNTER
    GEN_COUNTER = 0
    ...
    scenario_name = f"{safe_exp_name}_{period_key}_combined"
    scenario_dir = os.path.join(base_output_dir, scenario_name)
    os.makedirs(scenario_dir, exist_ok=True)
    
    print(f"\n  Creating COMBINED scenario: {scenario_name}")
    ...
    scenario_config = {
        'scenario': {
            'name': scenario_name,
            'description': f'Combined multi-DC scenario. {len(locations)} DCs x {dc_size}MW = {dc_size * len(locations):.0f}MW total. Period: {period["description"]}',
            'two_settlement': True
        },
        'simulation': {
    ...
    for location in locations:
        ...
        print(f"      Data Center: {dc_size} MW")
    ...
    print(f"\n    Written: {config_path}")
    print(f"    Total loads: {len(scenario_config['add_loads'])}")
...
def generate_scenarios_from_siting(
    ...
    """Generate combined multi-location scenarios from siting CSV."""
    global GEN_COUNTER
    GEN_COUNTER = 0
    
    print(f"\n{'='*60}")
    print("Generating COMBINED Multi-Location Scenarios")
    print("(All locations per experiment in ONE scenario)")
    print(f"{'='*60}")
    ...
    # Show summary of what we'll create
    for exp_name in exp_names[:3]:  # Show first 3
    ...
    for exp_name in exp_names:
        ...
        for period_key in periods:
            scenario_dir = create_combined_scenario(
                siting_df=siting_df,
                exp_name=exp_name,
    ...
    print(f"\n{'='*60}")
    print(f"Created {len(created_scenarios)} combined scenario(s)")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate combined multi-location scenario configurations',
    ...
    parser.add_argument(
        '--output-dir',
        default='scenarios_combined',
        help='Output directory for scenarios (default: scenarios_combined)'
    )
